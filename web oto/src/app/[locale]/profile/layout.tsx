...
export default function ProfileLayout({ children }: ProfileLayoutProps) {
  ...
  if (!mounted) {
    return (
      <>
        <PublicHeader />
        <div className="flex min-h-[60vh] items-center justify-center theme-page">
    ...
    <>
      ...
      <div className="min-h-screen theme-page">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          {/* Header Banner */}
          <div className="relative mb-8 overflow-hidden rounded-lg border theme-border bg-[radial-gradient(ellipse_at_top_right,rgba(227,24,55,0.1),transparent_60%),linear-gradient(to_bottom_right,var(--surface),var(--muted))] p-6 md:p-8 shadow-xl">
            <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="font-display text-2xl md:text-3xl font-extrabold tracking-tight text-[var(--foreground)]">
                    {user?.name || "Customer"}
                  </h1>
                  <p className="mt-1 text-sm text-[var(--subtle)] font-medium">
                    {user?.email || "customer@example.com"}
                  </p>
                </div>
              </div>
              <div className="flex">
                <span className="inline-flex items-center rounded-md bg-[#e31837]/15 px-3 py-1 text-xs font-bold uppercase tracking-wider text-[#e31837] border border-[#e31837]/30">
                  {user?.role || t("title")}
                </span>
              </div>
            </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <aside className="hidden lg:block lg:w-64 shrink-0">
              <div className="sticky top-24 rounded-lg border theme-border theme-surface p-4 shadow-md">
                      ...
                      <Link
                        ...
                        className={`flex items-center gap-3 rounded-md px-4 py-3 text-sm font-semibold tracking-wide transition-all duration-200 ${
                          item.active
                            ...
                            : "text-[var(--subtle)] hover:bg-[var(--muted)] hover:text-[var(--foreground)]"
                        }`}
                      >
                        <Icon size={18} className={item.active ? "text-white" : "text-[var(--subtle)]"} />
                        {item.label}
                      </Link>
                    );
                  })}
            ...
            <nav className="flex lg:hidden overflow-x-auto pb-3 gap-2 border-b theme-border scrollbar-none">
                  ...
                  <Link
                    ...
                    className={`flex items-center gap-2 whitespace-nowrap rounded-md px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-all duration-200 ${
                      item.active
                        ...
                        : "border theme-border theme-surface text-[var(--subtle)] hover:text-[var(--foreground)]"
                    }`}
                  >
                    <Icon size={14} />
                    {item.label}
              ...
              <button
                onClick={handleLogout}
                className="flex items-center gap-2 whitespace-nowrap rounded-md border border-red-500/30 theme-surface px-4 py-2.5 text-xs font-bold uppercase tracking-wider text-red-400 hover:bg-red-500/10"
              >
                <LogOut size={14} />
                {t("logout")}
              </button>
