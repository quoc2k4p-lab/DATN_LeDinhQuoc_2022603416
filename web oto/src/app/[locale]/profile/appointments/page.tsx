...
import { getMeAppointmentsAction, cancelAppointmentByCustomerAction } from "@/lib/actions/auth";
...
interface AppointmentItem {
  ...
  car_id: string;
  user_id: string | null;
  // Snapshot fields — preserved from booking time
...
export default function AppointmentsPage() {
  ...
  const handleCancelAppointment = async (id: string) => {
    ...
    setMessage(null);
    try {
      const res = await cancelAppointmentByCustomerAction(id);
      if (res.success) {
        setAppointments((prev) =>
          prev.map((apt) => (apt.id === id ? { ...apt, status: "cancelled" } : apt))
        );
        setMessage({
          text: locale === "vi" ? "Đã hủy lịch hẹn và gửi email thông báo thành công!" : "Appointment cancelled and notification emails sent successfully!",
          type: "success",
        });
      } else {
        setMessage({
          text: res.message || (locale === "vi" ? "Có lỗi xảy ra khi hủy lịch hẹn." : "An error occurred while cancelling the appointment."),
          type: "error",
        });
      }
    } catch (err) {
      ...
      setMessage({
        text: locale === "vi" ? "Có lỗi kết nối. Vui lòng thử lại sau." : "Connection error. Please try again later.",
        type: "error",
      });
    } finally {
  ...
  if (appointments.length === 0) {
    return (
      <div className="rounded-lg border theme-border theme-surface p-12 text-center shadow-md">
        <CalendarDays className="mx-auto h-12 w-12 theme-subtle mb-4" />
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6">
        {appointments.map((apt) => (
          <div
            key={apt.id}
            className="group relative overflow-hidden rounded-lg border theme-border theme-surface p-5 shadow-md transition duration-300 hover:border-[#e31837]/35"
          >
            ...
            <div className="flex flex-col md:flex-row gap-5">
              ...
              {/* Car Image */}
              <div className="relative h-32 w-full md:w-48 shrink-0 overflow-hidden rounded-md bg-[var(--muted)] border theme-border">
                {apt.car?.thumbnail ? (
                  <img
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between">
                <div>
                  ...
                  {/* Stats list */}
                  <div className="grid gap-x-6 gap-y-2 text-sm text-[var(--subtle)] theme-subtle mt-3 md:grid-cols-2">
                ...
                {/* Note section */}
                {apt.note && (
                  <div className="mt-4 rounded border theme-border bg-[var(--muted)] p-3 text-xs text-[var(--subtle)]">
                ...
                {(apt.status === "pending" || apt.status === "confirmed") && (
                  <div className="mt-4 flex justify-end">
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-md border border-red-600/30 bg-red-600/10 px-4 py-2 text-xs font-bold uppercase tracking-wider text-red-500 hover:bg-red-600 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition duration-200"
                    >
                      {actionLoading ? (
                        <Loader2 className="h-3 w-3 animate-spin mr-2" />
                      ) : null}
                      {locale === "vi" ? "Hủy lịch hẹn" : "Cancel Appointment"}
                    </button>
                  </div>
                )}
