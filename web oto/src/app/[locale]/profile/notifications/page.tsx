  ...
  if (notifications.length === 0) {
    return (
      <div className="rounded-lg border theme-border theme-surface p-12 text-center shadow-md">
        <Bell className="mx-auto h-12 w-12 theme-subtle mb-4" />
        <h3 className="font-display text-lg font-bold text-[var(--foreground)]">{t("noNotifications")}</h3>
        <p className="mt-2 text-sm theme-subtle max-w-sm mx-auto">
          {locale === "vi"
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className={`text-sm tracking-tight ${
                    notif.is_read === 0 ? "font-bold text-[var(--foreground)]" : "font-semibold text-[var(--subtle)]"
                  }`}>
                    {notif.title}
                  </h3>
                  <p className="mt-1 text-sm theme-subtle leading-relaxed">
                    {notif.content}
                  </p>
                  <span className="mt-2 block text-[10px] uppercase font-bold tracking-wider text-[var(--subtle)]/75">
              ...
              <div className="flex gap-2">
                {notif.link && (
                  (() => {
                    const isExternalOrRoot = notif.link.startsWith("/admin") || notif.link.startsWith("/staff") || notif.link.startsWith("/dashboard");
                    const TargetLinkComponent = isExternalOrRoot ? NextLink : Link;
                    return (
                      <TargetLinkComponent
                        href={notif.link}
                        onClick={() => notif.is_read === 0 && handleMarkAsRead(notif.id)}
                        className="inline-flex h-8 w-8 items-center justify-center rounded border theme-border bg-[var(--muted)] text-[var(--subtle)] hover:bg-[var(--foreground)] hover:text-[var(--background)] transition"
