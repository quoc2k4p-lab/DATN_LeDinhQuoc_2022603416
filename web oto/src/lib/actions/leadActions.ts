...
import { sendContactConfirmationEmail, sendContactNotificationToAdmin } from "@/lib/mailer";
...
export async function createContactRequestAction(formData: {
  ...
  try {
    ...
    // 6. Send confirmation email to customer and notification email to admin (wrapped in try-catch so it won't fail the transaction if API fails)
    try {
...
export async function assignLeadSalesAction(leadId: string, staffId: string | null): Promise<ActionResponse> {
  try {
    ...
    if (customerList.length > 0) {
      ...
      await pool.query(
        "UPDATE customers SET assigned_staff_id = ?, stage = 'contacted' WHERE id = ?;",
        [staffId, customerId]
      );
...
export async function updateLeadStageAction(leadId: string, stage: string): Promise<ActionResponse> {
  try {
    ...
    // Note: CRM stages map to customer stages (lead, contacted, test_drive, negotiating, purchased)
    let crmStage = "contacted";
    if (stage === "new_lead") crmStage = "lead";
    else if (stage === "assigned") crmStage = "contacted";
    else if (stage === "consulting") crmStage = "contacted";
    else if (stage === "appointment") crmStage = "test_drive";
    else if (stage === "quotation") crmStage = "negotiating";
    else if (stage === "purchased") crmStage = "purchased";
    else if (stage === "closed") crmStage = "contacted";
