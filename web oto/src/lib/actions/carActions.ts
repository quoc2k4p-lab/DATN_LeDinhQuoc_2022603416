...
import { crawlBonBanhCars, crawlOtoCars, crawlChoTotCars, CrawledCar, standardizeCarWithAI } from "@/lib/crawlService";
...
export async function createCarAction(formData: FormData): Promise<ActionResponse & { carId?: string }> {
  try {
    ...
    const carSortOrder = parseInt(formData.get("sortOrder")?.toString() || "0", 10);
 ...
 */
export async function crawlCarsAction(
  source: "bonbanh" | "oto" | "chotot" = "bonbanh",
  brand?: string,
  condition: "new" | "used" = "used"
): Promise<ActionResponse & { insertedCount?: number }> {
  try {
    let crawledCars: CrawledCar[] = [];
    if (source === "oto") {
      crawledCars = await crawlOtoCars(brand, condition);
    } else if (source === "chotot") {
      crawledCars = await crawlChoTotCars(brand, condition);
    } else {
      crawledCars = await crawlBonBanhCars(brand, condition);
    }
    console.log(`Crawl Action (${source}, brand: ${brand || "all"}, condition: ${condition}): Fetched ${crawledCars.length} cars.`);

    // Enhance crawled cars using Heuristics + Gemini AI sequentially to prevent rate limit errors
    if (crawledCars.length > 0) {
      console.log(`Standardizing ${crawledCars.length} crawled cars...`);
      const standardizedCars: CrawledCar[] = [];
      for (let i = 0; i < crawledCars.length; i++) {
        const car = crawledCars[i];
        console.log(`[Crawl Enrichment] Standardizing car ${i + 1}/${crawledCars.length}: "${car.title}"`);
        const start = Date.now();
        const enriched = await standardizeCarWithAI(car);
        standardizedCars.push(enriched);
        const duration = Date.now() - start;
        // If it took longer than 150ms, it probably made a network request to the Gemini API. Sleep 1.5 seconds.
        if (duration > 150 && i < crawledCars.length - 1) {
          console.log(`  Hit Gemini API. Sleeping 1500ms to avoid rate limits...`);
          await new Promise((resolve) => setTimeout(resolve, 1500));
        }
      }
      crawledCars = standardizedCars;
    }
