...
export async function createCrmLead(input: LeadInput): Promise<{ customerId: string; isNew: boolean }> {
  ...
  if (existingList.length > 0) {
    ...
    await pool.query(
      ...
      ) VALUES (?, ?, ?, ?, ?, NULL, 'new_lead', ?, NULL, 'AI_CHATBOT', 'active')`,
      [
        customerId,
        input.name.trim(),
        cleanPhone,
...
export async function createCrmAppointment(input: AppointmentInput): Promise<string> {
  ...
  await pool.query(
    `INSERT INTO appointments (id, car_id, user_id, customer_name, customer_phone, customer_email, appointment_date, note, status)
     VALUES (?, ?, NULL, ?, ?, ?, ?, ?, 'pending');`,
    [
      id,
      input.carId,
