...
- SO SÁNH XE (compareCars) hoặc XEM CHI TIẾT (getCarDetails): Các công cụ này yêu cầu truyền vào "id" chính xác của xe dưới dạng UUID. Nếu người dùng chỉ nói tên xe (ví dụ: "Toyota Camry", "Acura MDX"), bạn KHÔNG ĐƯỢC đoán ID của chúng. Hãy gọi "getAvailableCars" hoặc "searchCars" trước để lấy danh sách xe hiện tại cùng ID của chúng, sau đó lọc ra ID khớp và gọi "compareCars" hoặc "getCarDetails".
- TÍNH TRẢ GÓP (calculateLoan): Yêu cầu tham số "price" (giá trị xe). Nếu chưa biết giá xe người dùng muốn mua, hãy tìm giá xe đó bằng "searchCars" hoặc "getCarDetails" trước, sau đó mới gọi "calculateLoan". Nếu người dùng không chỉ rõ, mặc định tỷ lệ trả trước (downPayment) là 30, thời hạn vay (term) là 60 tháng, lãi suất (interestRate) là 8.5%/năm.
- ĐẶT LỊCH HẸN (createAppointment): Yêu cầu đầy đủ các thông tin: "carId" (lấy từ dữ liệu xe), "customerName" (tên khách hàng), "phone" (số điện thoại), "email" (email), và ngày hẹn "date" (chuỗi định dạng ISO, ví dụ: 2026-06-12T09:00:00+07:00). Hãy khéo léo xin khách hàng các thông tin này trước khi gọi hàm.`;
...
export async function generateAiChatResponse(
  ...
  const dynamicSystemPrompt = `${SYSTEM_PROMPT}\n\n[THỜI GIAN HỆ THỐNG HIỆN TẠI]: Ngày ${currentTimeStr} (Múi giờ Việt Nam). Hãy sử dụng mốc thời gian này để tính toán ngày giờ khi người dùng yêu cầu đặt lịch hẹn (ví dụ: "ngày mai", "tuần sau", "thứ hai tới").`;
  ...
  try {
    ...
    while (modelPart?.functionCall && loopCount < 3) {
      ...
      if (!response.ok) {
        console.error("Gemini API Loop Error details:", resJson);
        throw new Error(resJson.error?.message || "Lỗi gọi API Gemini trong vòng lặp");
      }
