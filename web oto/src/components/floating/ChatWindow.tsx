...
export function ChatWindow({ isOpen, onClose, onUnreadChange }: ChatWindowProps) {
  ...
  return (
    <div
      style={viewportStyle}
      className="fixed bottom-0 right-0 z-50 flex h-[100dvh] w-full flex-col rounded-none bg-[var(--surface)]/95 shadow-2xl backdrop-blur-xl transition-all duration-300 sm:bottom-24 sm:right-6 sm:w-[380px] sm:h-[500px] sm:rounded-xl sm:border border-[var(--line)] overflow-hidden"
    >
      ...
      <div className="flex items-center justify-between border-b border-[var(--line)] px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-display text-sm font-extrabold text-[var(--foreground)]">
              {isConnected ? "Hỗ trợ trực tuyến" : "Mất kết nối..."}
            </h4>
            <p className="text-[10px] font-semibold text-[var(--subtle)] uppercase tracking-wider">
        ...
        <button 
          onClick={onClose}
          className="rounded-full p-1.5 text-[var(--subtle)] hover:bg-[var(--muted)] hover:text-[var(--foreground)] transition"
        ...
        {!isRegistered ? (
          ...
          <form onSubmit={handleRegister} className="flex h-full flex-col justify-center space-y-4">
            ...
            <label className="block">
              <span className="mb-1.5 block text-xs font-bold uppercase tracking-[0.12em] text-[var(--subtle)]">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-md border border-[var(--line)] bg-[var(--background)] pl-10 pr-4 text-base sm:text-sm text-[var(--foreground)] placeholder-[var(--subtle)]/50 outline-none transition focus:border-[#e31837]"
                />
              </div>
            </label>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {messages.length === 0 ? (
              <div className="flex flex-1 flex-col items-center justify-center text-center">
                ...
                <p className="text-sm font-semibold text-[var(--foreground)]">Gửi lời chào của bạn!</p>
                <p className="mt-1 text-xs text-[var(--subtle)] max-w-[240px]">
              ...
              <div className="flex-1 space-y-3 overflow-y-auto pr-1">
                {messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[10px] font-bold text-[var(--subtle)] mb-1 px-1">
                      ...
                      <div
                        className={`rounded-lg px-4 py-2.5 text-sm max-w-[85%] break-words leading-relaxed shadow-sm ${
                          isMe
                            ...
                            : "bg-[var(--surface-strong)] text-[var(--foreground)] rounded-tl-none border border-[var(--line)]"
                        }`}
                      >
                        {msg.message_text}
                      </div>
                      ...
                      <div className="flex items-center gap-1 mt-1 px-1">
                        <span className="text-[9px] text-[var(--subtle)]/70">
                        ...
                        {isMe && (
                          msg.is_read === 1 ? (
                            ...
                            <Check size={10} className="text-[var(--subtle)]/70" />
                          )
                        )}
                      </div>
                    </div>
                ...
                {isStaffTyping && (
                  <div className="flex flex-col items-start mt-2">
                    ...
                    <div className="flex items-center gap-1.5 rounded-lg px-4 py-2.5 text-sm bg-[var(--surface-strong)] text-[var(--subtle)] rounded-tl-none border border-[var(--line)]">
      ...
      {/* Input Area */}
      {isRegistered && (
        <form onSubmit={handleSendMessage} className="border-t border-[var(--line)] bg-[var(--surface-strong)] p-4 flex gap-2">
          <input
            type="text"
            value={inputText}
            onChange={(e) => {
            ...
            className="h-11 flex-1 rounded-md border border-[var(--line)] bg-[var(--background)] px-4 text-base sm:text-sm text-[var(--foreground)] placeholder-[var(--subtle)]/70 outline-none transition focus:border-[#e31837] disabled:opacity-50"
          />
          <button
            type="submit"
