...
import { DbCar, DbCarImage, DbUser, DbCustomer } from "@/lib/db";
...
export function CarForm({ initialCar, initialImages, staffList = [], customerList = [] }: CarFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    // SOLD SPECIFICATIONS
    soldPrice: initialCar?.sold_price?.toString() || initialCar?.price?.toString() || "",
    soldBy: initialCar?.sold_by || "",
    buyerId: initialCar?.buyer_id || "",
    sortOrder: initialCar?.sort_order?.toString() || "0",
  });
  ...
  const handleChange = (
    ...
    if (name === "carCondition") {
      ...
      setFormData((prev) => ({
        ...prev,
        status: value,
        soldPrice: prev.soldPrice || prev.price || "",
      }));
    } else {
  ...
  const handleDateChange = (date: string) => {
    setFormData((prev) => {
      const time = prev.reservedUntil && prev.reservedUntil.includes("T") ? prev.reservedUntil.split("T")[1] : "";
      return {
        ...prev,
        reservedUntil: date ? (time ? `${date}T${time}` : date) : "",
      };
    });
    if (errors.reservedUntil) {
      setErrors((prev) => {
        const next = { ...prev };
  ...
  const handleTimeChange = (time: string) => {
    setFormData((prev) => {
      const date = prev.reservedUntil ? prev.reservedUntil.split("T")[0] : toDateTimeLocalString(new Date()).split("T")[0];
      return {
        ...prev,
        reservedUntil: date ? (time ? `${date}T${time}` : date) : "",
      };
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.title_vi.trim()) clientErrors.title_vi = "Tên xe là bắt buộc";
    ...
    if (!formData.description_vi.trim()) clientErrors.description_vi = "Mô tả xe là bắt buộc";
    if (!isEdit && !thumbnailFile) clientErrors.thumbnail = "Ảnh đại diện là bắt buộc";

    if (formData.status === "sold") {
      if (!formData.soldPrice.trim() || parseInt(formData.soldPrice) <= 0) {
        clientErrors.soldPrice = "Giá bán thực tế phải lớn hơn 0";
      }
      if (!formData.soldBy) {
        clientErrors.soldBy = "Nhân viên bán hàng là bắt buộc";
      }
    }

    if (formData.status === "reserved") {
      if (!formData.reservedUntil) {
        clientErrors.reservedUntil = "Thời gian giữ cọc là bắt buộc";
      } else if (!formData.reservedUntil.includes("T")) {
        clientErrors.reservedUntil = "Vui lòng chọn cả ngày và giờ giữ cọc";
      } else {
        const reservedDate = new Date(formData.reservedUntil);
        if (isNaN(reservedDate.getTime()) || reservedDate < new Date()) {
          clientErrors.reservedUntil = "Thời gian giữ cọc phải lớn hơn thời gian hiện tại";
        }
      }
    }
    ...
    startTransition(async () => {
      ...
      if (formData.status === "sold") {
        submissionData.append("soldPrice", formData.soldPrice);
        submissionData.append("soldBy", formData.soldBy);
        submissionData.append("buyerId", formData.buyerId);
      } else {
        submissionData.append("soldPrice", "");
        submissionData.append("soldBy", "");
        submissionData.append("buyerId", "");
      }
      ...
      // Find remaining existing gallery URLs (not data URLs)
      const existingUrls = galleryPreviews.filter((url) => !url.startsWith("data:"));
      existingUrls.forEach((url) => {
        submissionData.append("existingGallery", url);
      });
      ...
      <div className="rounded-md border border-white/10 bg-[#151a22] p-6 space-y-6">
        ...
        <div className="grid gap-5">
          ...
          <label className="block">
            <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Tên xe *</span>
            <input
              name="title_vi"
        ...
        {/* Mô tả */}
        <label className="block">
          <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Mô tả chi tiết *</span>
          <textarea
            name="description_vi"
      ...
      <aside className="space-y-6">
        ...
        <div className="rounded-md border border-white/10 bg-[#151a22] p-6">
          ...
          >
            ...
            <option value="reserved">Đang giữ cọc</option>
          ...
          {formData.status === "reserved" && (
            <div className="mt-4 border-t border-white/5 pt-4 space-y-3 forced-dark">
              <DateTimePicker
                selectedDate={formData.reservedUntil ? formData.reservedUntil.split("T")[0] : ""}
                selectedTime={formData.reservedUntil && formData.reservedUntil.includes("T") ? formData.reservedUntil.split("T")[1] : ""}
                onDateChange={handleDateChange}
                onTimeChange={handleTimeChange}
                dateError={errors.reservedUntil}
                singleColumn={true}
                dateLabel="Ngày giữ cọc"
                timeLabel="Giờ giữ cọc"
              />
          ...
          {formData.status === "sold" && (
            <div className="mt-4 border-t border-white/5 pt-4 space-y-4">
              <div>
                <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Giá bán thực tế (VNĐ) *</span>
                <input
                  type="number"
                  name="soldPrice"
                  value={formData.soldPrice}
                  onChange={handleChange}
                  placeholder="Ví dụ: 1080000000"
                  className={`h-12 w-full rounded-md border bg-[#080c11] px-4 text-zinc-100 outline-none transition focus:border-[#e31837] ${
                    errors.soldPrice ? "border-red-500 bg-red-500/5" : "border-white/10"
                  }`}
                />
                {errors.soldPrice && <span className="mt-1 text-xs text-red-500">{errors.soldPrice}</span>}
              </div>

              <div>
                <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Nhân viên bán hàng *</span>
                <select
                  name="soldBy"
                  value={formData.soldBy}
                  onChange={handleChange}
                  className={`h-12 w-full rounded-md border bg-[#080c11] px-4 text-zinc-100 outline-none transition focus:border-[#e31837] ${
                    errors.soldBy ? "border-red-500 bg-red-500/5" : "border-white/10"
                  }`}
                >
                  <option value="">-- Chọn nhân viên --</option>
                  {staffList.map((staff) => (
                    <option key={staff.id} value={staff.id}>
                      {staff.full_name} ({staff.role === "admin" ? "Admin" : "Nhân viên"})
                    </option>
                  ))}
                </select>
                {errors.soldBy && <span className="mt-1 text-xs text-red-500">{errors.soldBy}</span>}
              </div>

              <div>
                <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Khách hàng mua xe</span>
                <select
                  name="buyerId"
                  ...
                  className="h-12 w-full rounded-md border border-white/10 bg-[#080c11] px-4 text-zinc-100 outline-none transition focus:border-[#e31837]"
                >
                  <option value="">-- Khách vãng lai (Không có trong CRM) --</option>
        ...
        {/* Thứ tự sắp xếp */}
        <div className="rounded-md border border-white/10 bg-[#151a22] p-6">
          <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Thứ tự ưu tiên</span>
          <input
            type="number"
            name="sortOrder"
            value={formData.sortOrder}
            onChange={handleChange}
            placeholder="Số lớn hơn sẽ xếp lên trước (mặc định: 0)"
            className="h-12 w-full rounded-md border border-white/10 bg-[#080c11] px-4 text-zinc-100 outline-none transition focus:border-[#e31837]"
          />
          <p className="mt-2 text-xs text-zinc-500">Số càng lớn (ví dụ: 10, 5, 1) xe sẽ càng được ưu tiên đưa lên đầu trang chủ và danh sách xe.</p>
        </div>
