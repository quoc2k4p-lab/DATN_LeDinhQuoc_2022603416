...
export function DateTimePicker({
  ...
  return (
    <div className="md:col-span-2">
      <div className={singleColumn ? "grid gap-3" : "grid md:grid-cols-[1fr_180px] gap-3"}>
        {/* ====== COMPACT CALENDAR ====== */}
        <div>
          ...
          <div className="rounded-lg border theme-border bg-[var(--background)] overflow-hidden">
            {/* Month header */}
            <div className="flex items-center justify-between px-3 py-2 sm:py-1.5 border-b theme-border bg-gradient-to-r from-[#e31837]/5 to-transparent">
              <button type="button" onClick={goToPrevMonth} disabled={!canGoPrev}
                className="p-2 sm:p-1 rounded hover:bg-[var(--foreground)]/5 transition disabled:opacity-20 disabled:cursor-not-allowed" aria-label="Tháng trước">
                <ChevronLeft className="w-4 h-4" />
              </button>
              <span className="text-sm sm:text-xs font-semibold tracking-wide">
                {MONTHS_VI[viewMonth]} {viewYear}
              </span>
              <button type="button" onClick={goToNextMonth}
                className="p-2 sm:p-1 rounded hover:bg-[var(--foreground)]/5 transition" aria-label="Tháng sau">
            ...
            <div className="grid grid-cols-7">
              {WEEKDAYS_VI.map((wd) => (
                <div key={wd} className="py-1.5 sm:py-1 text-center text-xs sm:text-[10px] font-bold uppercase tracking-wider theme-subtle">
                  {wd}
                </div>
              ))}
            </div>
            ...
            {/* Day grid */}
            <div className="grid grid-cols-7 px-1 pb-1 gap-1 sm:gap-0.5">
              {calendarCells.map((cell, idx) => {
                if (cell.day === 0) return <div key={`b-${idx}`} className="w-full h-10 sm:h-8" />;
                ...
                return (
                  <button key={cell.dateStr} type="button" disabled={disabled} onClick={() => handleDateClick(cell.dateStr)}
                    className={`
                      h-10 sm:h-8 flex items-center justify-center rounded text-sm sm:text-xs font-medium transition-all duration-150
                ...
                {TIME_SLOTS.map(({ label, hour }) => {
                  ...
                  return (
                    <button key={label} type="button" disabled={disabled} onClick={() => handleTimeClick(label)}
                      className={`
                        rounded-md py-3 sm:py-2 px-1 text-center transition-all duration-150 border
                        ${disabled
                          ? status === "full"
                      ...
                      `}>
                      <span className={`text-sm sm:text-xs font-semibold ${selected ? "text-white" : ""}`}>{label}</span>
                      {!disabled && !selected && (
                        <span className={`block text-[10px] sm:text-[9px] leading-none mt-0.5 ${status === "limited" ? "text-amber-500/70" : "text-emerald-500/50"}`}>
