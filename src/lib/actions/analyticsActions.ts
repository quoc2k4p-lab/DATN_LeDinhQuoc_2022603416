...
export async function markCarAsSold(
  ...
  try {
    ...
    // Update customer stage if buyer provided
    if (buyerId) {
      await pool.query(
    ...
    return { success: true, message: "Cập nhật trạng thái bán xe thành công." };
  } catch (error) {
    console.error("Error marking car as sold:", error);
    return { success: false, message: "Không thể cập nhật trạng thái xe." };
  }
}
