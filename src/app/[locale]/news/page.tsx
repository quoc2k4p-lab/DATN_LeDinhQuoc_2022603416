...
import { ArrowRight, Calendar, User, ArrowLeft, ArrowRight as NextIcon } from "lucide-react";
...
export default async function NewsPage({ searchParams }: NewsPageProps) {
  ...
  // Determine featured post (only on page 1 with no search/category filter active)
  const showFeatured = currentPage === 1 && !q && (!category || category === "Tất cả");
  let featuredPost: DbPost | null = null;
  let displayPosts = [...filteredPosts];

  if (showFeatured && filteredPosts.length > 0) {
    // Find featured article or fallback to latest
    const foundFeatured = filteredPosts.find((p) => p.featured === 1);
    featuredPost = foundFeatured || filteredPosts[0];
    
    // Exclude featured article from the general listing
    displayPosts = filteredPosts.filter((p) => p.id !== featuredPost?.id);
  }
