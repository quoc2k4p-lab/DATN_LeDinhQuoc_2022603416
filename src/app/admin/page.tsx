...
export default async function AdminDashboardPage() {
  ...
  const [customersRes] = await pool.query(`
    SELECT COUNT(DISTINCT email) as count FROM (
      ...
      UNION
      SELECT customer_email as email FROM appointments
    ) as temp;
  `);
  ...
  const [hotLeadsRes] = await pool.query(`
    SELECT COUNT(DISTINCT a.customer_email) as count 
    FROM appointments a 
