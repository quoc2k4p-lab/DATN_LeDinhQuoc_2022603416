...
export function AppointmentForm({ cars, initialCarId }: AppointmentFormProps) {
  ...
  const defaultCarName = cars.find(c => c.id === initialCarId || c.slug === initialCarId)?.name || cars[0]?.name || "";
  ...
  useEffect(() => {
    if (cars.length > 0) {
      const selected = cars.find(c => c.id === initialCarId || c.slug === initialCarId) || cars[0];
      setFormData(prev => ({
        ...prev,
        carId: selected.id,
      }));
  ...
  const handleChange = (
    ...
    if (name === "carName") {
      const matchedCar = cars.find(c => c.name === value);
      if (matchedCar) {
