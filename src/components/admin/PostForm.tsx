...
export function PostForm({ initialPost }: PostFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    excerpt_en: initialPost?.excerpt_en || "",
    content_vi: initialPost?.content_vi || initialPost?.content || "",
    content_en: initialPost?.content_en || "",
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.title_vi.trim()) clientErrors.title_vi = "Tiêu đề bài viết (Tiếng Việt) là bắt buộc";
    if (!formData.title_en.trim()) clientErrors.title_en = "Tiêu đề bài viết (Tiếng Anh) là bắt buộc";
    if (!formData.slug.trim()) clientErrors.slug = "Đường dẫn slug là bắt buộc";
    if (!formData.excerpt_vi.trim()) clientErrors.excerpt_vi = "Tóm tắt (Tiếng Việt) là bắt buộc";
    if (!formData.excerpt_en.trim()) clientErrors.excerpt_en = "Tóm tắt (Tiếng Anh) là bắt buộc";
    if (!formData.content_vi.trim()) clientErrors.content_vi = "Nội dung (Tiếng Việt) là bắt buộc";
    if (!formData.content_en.trim()) clientErrors.content_en = "Nội dung (Tiếng Anh) là bắt buộc";
    ...
    startTransition(async () => {
      ...
      submissionData.append("excerpt_en", formData.excerpt_en);
      submissionData.append("content", formData.content_vi);
      submissionData.append("content_vi", formData.content_vi);
      submissionData.append("content_en", formData.content_en);
      ...
      <div className="rounded-md border border-white/10 bg-[#151a22] p-6 space-y-6">
        ...
        <div className="space-y-5">
          ...
          <label className="block">
            <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Tiêu đề bài viết (Tiếng Việt) *</span>
            <input
              name="title_vi"
          ...
          {/* Tiêu đề EN */}
          <label className="block">
            <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Tiêu đề bài viết (Tiếng Anh) *</span>
            <input
              name="title_en"
              value={formData.title_en}
              onChange={handleChange}
              placeholder="Ví dụ: Detailed Review of Honda Civic RS 2025"
              className={`h-12 w-full rounded-md border bg-[#080c11] px-4 text-zinc-100 outline-none transition focus:border-[#e31837] ${
          ...
          {/* Tóm tắt VI */}
          <label className="block">
            <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Tóm tắt ngắn (Tiếng Việt) *</span>
            <textarea
              name="excerpt_vi"
              value={formData.excerpt_vi}
              onChange={handleChange}
              placeholder="Viết một đoạn tóm tắt ngắn bằng Tiếng Việt..."
          ...
          {/* Tóm tắt EN */}
          <label className="block">
            <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Tóm tắt ngắn (Tiếng Anh) *</span>
            <textarea
              name="excerpt_en"
              ...
              className={`min-h-20 w-full rounded-md border bg-[#080c11] p-4 text-zinc-100 outline-none transition focus:border-[#e31837] ${
                errors.excerpt_en ? "border-red-500 bg-red-500/5" : "border-white/10"
              }`}
            />
            {errors.excerpt_en && <span className="mt-1 text-xs text-red-500">{errors.excerpt_en}</span>}
          </label>

          {/* Nội dung chính VI */}
          <label className="block">
            <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Nội dung bài viết (Tiếng Việt) *</span>
            <textarea
              name="content_vi"
              value={formData.content_vi}
              onChange={handleChange}
              placeholder="Nhập nội dung bài viết bằng Tiếng Việt. Bạn có thể sử dụng cú pháp markdown..."
          ...
          {/* Nội dung chính EN */}
          <label className="block">
            <span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Nội dung bài viết (Tiếng Anh) *</span>
            <textarea
              name="content_en"
              value={formData.content_en}
              onChange={handleChange}
              placeholder="Enter article content in English. You can use markdown syntax..."
              className={`min-h-96 w-full rounded-md border bg-[#080c11] p-4 font-mono text-zinc-100 text-sm outline-none transition focus:border-[#e31837] ${
                errors.content_en ? "border-red-500 bg-red-500/5" : "border-white/10"
              }`}
            />
            {errors.content_en && <span className="mt-1 text-xs text-red-500">{errors.content_en}</span>}
          </label>
        </div>
